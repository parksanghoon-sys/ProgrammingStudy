...
ICollection 인터페이스는 IEnumerable 인터페이스를 기반으로 확장된 인터페이스 이다. c# 에서 제공되는 컬랙션 개체가 foreach 구문을 통해 보관된 각 요소에 공통된 작업을 수행할 수 있는것이 내부적으로 IEumerable 인터페이스를 기반으로 정의 되어 있기 때문이다.  
<br>
IEnumerable 인터페이스는 foreach에 필요한 맴버들을 약속된 IEnumerator 개체를 반환한는 `GetEnumerator`메서드를 제공하고 있다.  

▶ IEnumerable , IEnumerator 인터페이스의 약속된 멤버
```c#
...
```
강의생과 교수 관련 클래스를 정의하고 보관하는 컬랙션을 정의 해보자
```c#
