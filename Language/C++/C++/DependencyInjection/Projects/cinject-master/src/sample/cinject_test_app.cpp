...
int main()
{
    ...
    std::cout << "Resolving Human" << std::endl;
    std::shared_ptr<Human> humman = c.get<Human>();
    humman->jump();

    std::cout << "Resolving Snake" << std::endl;
    std::shared_ptr<Snake> snake = c.get<Snake>();
    snake->crawl();

    std::cout << "Resolving IWalker" << std::endl;
    std::shared_ptr<IWalker> walter = c.get<IWalker>();
    IWalker* ptr = walter.get();
    std::cout << "Address of underlying Walker object: " << ptr << std::endl;
    walter->walk();
